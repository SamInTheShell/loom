...
(view = read-only, write = read-write, at `/mnt/<name>`), and whether the
container gets **network** (off by default). A new terminal inherits the
...
the shell into another image, the **network pill** toggles access, and
