...
Everything configurable lives in it — the Servers tab, chat defaults,
permission modes, container definitions. It re-parses on every save; the
Servers tab and open chats pick changes up immediately. If it stops
parsing, the Servers tab shows the exact error until it parses again.
