...
Practical llama-server knowledge for the command line YOU run it with
(Loom only talks to the resulting API). All of it is ordinary
llama.cpp - anything its docs cover works. Remember `--jinja`: tool
calling needs the jinja template engine, and without it an agent
...
- `-fa on` - flash attention; faster and leaner, keep it on when your
  build supports it.
- `-c <tokens>` - the context window; Loom reads the served size from
  the API and shows it everywhere. Bigger windows cost VRAM in the KV
  cache; see below.
...
1. Look at llama-server's own output where you launched it - it prints
   layer offload counts, KV-cache sizes, and template warnings at
   startup; most mysteries are answered there.
2. Check the chat stats row (and the Diagnostics view from the context
   chip): low tok/s with high prompt time usually means CPU spill
   (`-ngl` too low or VRAM exhausted); a low cache-hit rate means the
   prompt prefix keeps changing.
